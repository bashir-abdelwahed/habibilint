...
class CompatibilityTest(TestCase):
    ...
    reported without habibilint. (That is, habibilint
    ...
    def create_habibilint_setup(self, strictness=None):
        try:
    ...
    def create_no_habibilint_setup(self):
        try:
            with open(".darglint", "w") as fout:
                fout.write("[darglint]\nignore=*\n")
        except Exception:
    ...
    def record_habibilint_errors(self, collection, filename):
        proc = subprocess.run(
            [
                "habibilint",
                filename,
    ...
    def test_with_habibilint_is_superset(self):
        errors = defaultdict(lambda: set())  # type: Dict[str, Set[str]]
        self.create_no_habibilint_setup()
    ...
    def test_defaults_taken_from_habibilint_config(self):
        self.create_habibilint_setup(strictness="short")
        ...
        # The error D100, which isn't a part of habibilint,
        # appears in flake8 with the use of a config file.
        # So, we need to remove it.
        flake8_errors[self.STRICTNESS_FILE].discard("D100")

        habibilint_errors = defaultdict(lambda: set())
        self.record_habibilint_errors(habibilint_errors, self.STRICTNESS_FILE)

        self.assertEqual(
            flake8_errors[self.STRICTNESS_FILE],
            habibilint_errors[self.STRICTNESS_FILE],
        )

    def test_defaults_overriden_from_habibilint_config(self):
        self.create_habibilint_setup(strictness="short")
