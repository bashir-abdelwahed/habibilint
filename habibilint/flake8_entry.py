...
class HabibilintChecker(object):
    name = "flake8-habibilint"
    version = __version__
    config = get_config()
    ...
    def run(self):
        ...
        # TODO: the typing above is wrong, it should be:
        #  Generator[Tuple[int, int, str, type], None, None]
        #  it means lineno, offset, message, class (is not used by flake8)
        #  for example, 1, 0, "DAR101: Missing docstring in public module", type(self)
        ...
        try:
            ...
            yield (
                last_line,
                0,
                "DAR000: Unexpected exception in habibilint: " + str(ex),
                type(self),
            )
