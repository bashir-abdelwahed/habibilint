...
def get_logger():  # type: () -> Logger
    """Get the default logger for habibilint.

    Returns:
        The default logger for habibilint.

    """
    return logging.getLogger("habibilint")

# TODO: add .habibilint file
...
def load_config_file(filename):  # type: (str) -> Configuration
    ...
    if "darglint" in config.sections():
        lint_config = config["darglint"]
        if "ignore" in lint_config:
            errors = lint_config["ignore"]
            for error in errors.split(","):
                ignore.append(error.strip())
        if "enable" in lint_config:
            to_enable = lint_config["enable"]
            for error in to_enable.split(","):
                enable.append(error.strip())
        if "message_template" in lint_config:
            message_template = lint_config["message_template"]
        if "ignore_regex" in lint_config:
            ignore_regex = lint_config["ignore_regex"]
        if "ignore_raise" in lint_config:
            to_ignore_raise = lint_config["ignore_raise"]
            for exception in to_ignore_raise.split(","):
                ignore_raise.append(exception.strip())
        if "ignore_properties" in lint_config:
            ignore_properties = bool(lint_config["ignore_properties"])
        if "docstring_style" in lint_config:
            raw_style = lint_config["docstring_style"]
            style = DocstringStyle.from_string(raw_style)

        if "strictness" in lint_config:
            raw_strictness = lint_config["strictness"]
            strictness = Strictness.from_string(raw_strictness)

        if "indentation" in lint_config:
            try:
                indentation = int(lint_config["indentation"])
            except ValueError:
                raise Exception(
                    "Unrecognized value for indentation.  Expected "
                    "a non-zero, positive integer, but received {}".format(
                        lint_config["indentation"]
                    )
                )

        if "log_level" in lint_config:
            log_level = LogLevel.from_string(lint_config["log_level"])
