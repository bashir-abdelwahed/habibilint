"""Defines the command line interface for ."""
...
import habibilint.errors
from habibilint.error_report import ErrorReport
...
parser.add_argument(
    ...
    action="store_true",
    default=False,
    help="Class property methods will be ignored by ",
)
parser.add_argument(
    "--raise-syntax",
    action="store_true",
    help=(
        ...
        "rather than storing the error.  Useful for debugging ."
    ),
)
parser.add_argument(
    "--verbosity",
...
parser.add_argument(
    "--version",
    action="store_true",
    help=("Return the current version number of ."),
)
parser.add_argument(
    "files",
    nargs="*",
...
parser.add_argument(
    ...
    action="store_true",
    help=(
        "Exit with status 0, even on errors.  By default,  "
...
parser.add_argument(
    ...
    type=int,
    default=4,
    help=("The number of workers (processes) to run in parallel."),
)
...
def get_error_report(
    ...
    Args:
        ...
        raise_errors_for_syntax: True if we want parser errors
            to propagate up (crashing .)  This is useful
            if we are developing on  -- we can get the stack
            trace and know exactly where  failed.
    ...
    try:
        ...
        error = habibilint.errors.PythonSyntaxError(e)
...
def main():
    ...
    try:
        ...
        # exit code or not --  failed, and it should
        # look like it failed.
        logger = get_logger()
        logger.critical(exc)
        sys.exit(129)
